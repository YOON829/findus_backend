...
const getAllWorks = async (req, res) => {
  try {
    const works = await db.Work.findAll({
      attributes: ['work_name'], // 작품 이름만 가져옵니다.
      group: ['work_name'] // 같은 이름의 작품을 그룹화합니다.
    });
    // 작품 이름을 기반으로 작품 키(work_key)를 추가합니다.
...
const getWorkDetails = async (req, res) => {
  try {
    ...
    const seasons = await db.Work.findAll({
      where: { work_name: koreanWorkName },
      attributes: ['work_season'], // 시즌 정보만 가져옵니다.
      group: ['work_season'],
      order: [['work_season', 'ASC']] // 시즌을 오름차순으로 정렬합니다.
    });

    // 시즌 정보에 display_name을 추가하여 처리합니다.
    ...
    res.json({
      work: { ...work.toJSON(), work_key: workKey }, // 작품 정보에 작품 키를 추가합니다.
      seasons: processedSeasons // 처리된 시즌 정보를 반환합니다.
    });
  } catch (error) {
...
const getSeasonEpisodes = async (req, res) => {
  try {
    ...
    const episodes = await db.Work.findAll({
      ...
      attributes: ['work_name', 'work_season', 'description', 'poster', 'start_date', 'end_date', 'genre', 'director', 'series_writer'], // 필요한 필드만 가져옵니다.
...
const getSeasonDisplayName = (season) => {
  switch(season.toLowerCase()) {
    case '극장판': return 'movie'; // 극장판 -> movie
    case 'ova': return 'OVA';      // OVA -> OVA
    case 'pv': return 'PV';        // PV -> PV
    case '103기': return '103기';
    case '104기': return '104기';
    case '103기 페스라이브': return '103기 페스라이브';
    case '104기 페스라이브': return '104기 페스라이브';
    default: return `${season}`;   // 변환이 필요 없는 경우 그대로 반환
  }
};
...
const getSeasonDetails = async (req, res) => {
  try {
    ...
    const seasonDetails = await db.Work.findOne({
      ...
      attributes: ['work_id', 'work_name', 'work_season', 'description', 'poster', 'start_date', 'end_date', 'genre', 'director', 'series_writer']  // work_id로 수정
    });
    ...
    const filteredPlaces = places.map(place => {
      const filteredImages = place.Images.filter(image => image.image_url.includes('/realPlace/'));
      return {
        ...place.toJSON(),
